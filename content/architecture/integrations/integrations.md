...
**Update (26-08, [[2026-08-26-stackworkz-agent-demo]]): the Stackworkz relationship now has a mechanism, and the May reading was right.**

The line above, recorded from the [[29-05-2026-stackworkz-meeting|29 May session]], said that permissions and user management live in the Stackworkz BFF and are the source of truth the [[agent-access-layer]] permission model must mirror. **That has been confirmed in detail**, and it is why [[open-questions]] #71 turned out to be a misreading rather than a contradiction.

- **The permission framework Stackworkz is building**: a backend API with roles; **four role templates** (TXN, bin sponsor, client self-issuing, client with a programme manager); **TXN governs the ceiling** with no customisation for TXN or bin-sponsor roles; **clients build their own roles** beneath it, standard ones provided for smaller clients and full construction for enterprise; **user-level overrides** on top. Michael is sending the framework document.
- **Code sharing has two options and no decision yet.** Either Novosapien works inside Stackworkz's repository on isolated branches with Stackworkz reviewing the pull requests, or the codebase is handed over for them to absorb. Ruan could not choose without seeing the generated components and knowing where the MCP server sits. **George sent the codebase immediately** with Michael's approval, and a **smaller technical session was agreed, ad hoc rather than diarised**.
- **Stackworkz's release path**, which the two cadences have to reconcile: own repository, own dev environment, link to Michael to check, then push to DT for staging and production. Novosapien deploys on push to `main`. Ruan: *"on our side we're 100% flexible."*
- **How generative UI renders, answering [[open-questions]] #35.** The agent does not generate component code, which George named *"a potential security nightmare"*. It calls a tool, builds the payload, and passes the **component name plus its data**, so the Console imports **its own** pre-built component. The agent can therefore use exactly the same component set as the Console, with small changes for dynamism.
- **The Novosapien stack, disclosed to Stackworkz**: AG-UI from CopilotKit (headless) between agent and UI, **LangChain deep agents** running the agent itself, deployed on **Cloud Run in a single Docker container**. George offered to rebuild in whatever Stackworkz uses: *"just point us in the right direction and we can rebuild it all."*
- **The Super Ultra prototype Novosapien holds is the final one.** No further work is being done on it; the only possible discrepancy is Figma mock-ups produced afterwards, which Michael will send.
- **Michael is circulating a crossover list**, the interjection points between the two builds, naming reporting and the agent inbox as pages that have to merge cleanly. **Stackworkz are being given access to TXN's vault.**
- **Stackworkz's posture for the session**, from Ruan: *"this process now is not to make decisions, just to understand what it looks like so we can help with decisions later."*

The open architectural question from this session is [[open-questions]] #74, whether the MCP server routes through the Console or straight to DT, and it gates the wire-in.

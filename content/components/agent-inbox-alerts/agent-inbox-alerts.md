...
> **27-08 update ([[2026-08-27-agentic-standup]]): the inbox has been built as a working concept and TXN endorsed it.** Michael: *"it looks great. I think exactly what we had in mind."* The demonstrated flow: an alert arrives, the agent investigates, builds a short report, and **proposes a plan** the operator approves, edits with free text, or declines. It shows its work as it executes and writes to an audit trail throughout. Entirely mocked, no real data and nothing executing.
>
> **Two design decisions worth preserving.** First, **declining requires a reason, and those reasons become training data**: aggregate them, and *"50 out of a hundred were declined because it needs a wider review"* tells you which prompt to change. George was explicit that this is deliberately **not autonomous learning yet**, and should only become so once it works with humans in the loop. Second, Michael prefers the inbox in the **left sidebar** over the original prototype's co-pilot placement, because *"you're more involved with it now rather than just ask it a question and go away"*.
>
> George's framing of the whole client-facing surface, which is worth carrying: three modes, **user-initiated ad hoc**, **scheduled**, and **agent-initiated human-validated**, built in isolation and mixed afterwards. *"Is it working with you or is it working for you?"* (Source: standup 2026-08-27)

...
>
> **27-08 update ([[2026-08-26-workflow-slate-build]]):** the catalogue is **43 tools**, generated from the same contract. By HTTP verb the split is 23 read and 20 write; **by gate it is 24 open and 19 gated**, and the two disagree deliberately on `pin_authenticate`, a POST that mints a short-lived session token and changes no domain record. The gate is keyed by tool **name** and maintained by hand for exactly that reason, and `assert_gates_are_reachable` turns a rename on the MCP side into a **boot failure** rather than a silently ungated destructive tool. The catalogue exports `STATE_EFFECT_EXCEPTIONS` so the divergence is visible where the schema is read. **This hardens the gate; it is not the permission model** named in Pilot Order deliverable 2, which still does not exist.

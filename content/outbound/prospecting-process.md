---
description: "TXN's seven-stage prospecting process: four discovery sub-scans, the early-stage watch list, Freshsales model, phased rollout, and compliance rules"
---

# TXN Prospecting Process (v0.6)

> **Source of truth:** `TXN_GTM_Prospecting_Process_v0.6.docx` in the outbound folder (`programming/txn/outbound`, mirrored at `shared/clients/txn/outbound`), delivered by Ian Johnson on 25 August 2026, superseding the v0.4 delivered on 24 August. This page is a readable mirror. Edit the source document, then re-mirror. Routed from [[outbound]].
...
Version 0.6   |   Author: I. Johnson   |   19/08/2026
...
| 0.5 | 19/08/2026 | I. Johnson | I. Johnson | Section 4.1.3 extended to reference Card-Program-Categories as the canonical card program taxonomy. Introduces a fourth Category-driven sub-scan alongside the existing sub-sector, use-case, and incumbent-customer sub-scans. The Card-Program-Categories reference note enumerates four top-level categories (Consumer, Commercial and B2B, Disbursement and Payout, Vertical-specialist) covering approximately 54 specific card program types, plus six structural axes (funding model, cardholder type, form factor, acceptance loop, currency, processing model). Discovery passes from v0.5 onward must consider all four categories rather than the Consumer / Commercial bias inherent to sub-sector-primary discovery. |
| 0.6 | 19/08/2026 | I. Johnson | I. Johnson | Added Section 4.1.5 Early-stage Watch List. Codifies a signal-driven monthly scan for very early-stage fintechs (pre-Series-A, pre-product-launch, in the 6-to-24-month founding-to-first-public-event window) that would otherwise be missed by discovery weighted toward existing card programs. Five signal source categories (accelerator alumni, fresh regulator licence, fintech media watch pieces, first payments hire, seed round referencing card issuance). New 10_Watch_List tab in Qualification Matrix v0.7 stores candidates pre-scoring; promote-to-Prospect on a defined trigger. Zero additional tooling cost. |
...
Backfill: the first four MVP markets (Poland, Czech Republic, Romania, Hungary) were scoped with sub-sector-primary discovery only. A supplemental use-case + adjacent-geo + incumbent-customer pass in July 2026 backfilled the missed candidates. From v0.4 forward, the three-part discovery pattern applies to every market discovery pass. v0.5 extends this to a four-part pattern by adding the category-driven sub-scan below.

### 4.1.4 Category-driven discovery
The canonical taxonomy of card program categories and specific card program types is held in Card-Program-Categories.docx at 07_GTM/04_Sales_Motion/Assets/Pipeline/. Read that document before running any discovery pass. It enumerates four top-level categories that any card program falls into: Consumer, Commercial and B2B, Disbursement and Payout, and Vertical-specialist. Beneath the four categories the document lists approximately 54 specific card program types (from neobank debit through fleet and fuel to humanitarian aid disbursement). It also sets out six structural axes on which any given program can be characterised: funding and credit model, cardholder type, form factor, acceptance loop, currency, and processing and decisioning model.
Why category matters as a discovery lens: sub-sector filters a company by what industry it operates in. Use case filters by which of the ten Vision use cases it fits. Category filters by which type of card program it runs (or plans to run). These three lenses do not always coincide. A neobank (sub-sector Digital Banking, use case Digital Banks) may run a Consumer program only, or it may also run Commercial and B2B (SME debit cards for its business banking customers) or Disbursement (payroll cards for its enterprise clients). Filtering only by sub-sector or only by use case can miss the second and third card programs entirely. Category-driven discovery makes the coverage explicit.
Operational rule: from v0.5 forward, every discovery pass must include a fourth sub-scan (Sub-scan 4) that walks the four categories and asks, for the target market, which specific program types have material EEA activity and which companies operate those programs. Consumer and Commercial and B2B are the natural areas of coverage bias; Disbursement and Payout and Vertical-specialist typically under-surface unless deliberately targeted. Fleet and fuel, mobility, healthcare, cannabis and gaming, humanitarian aid, and creator payouts are all Vision-relevant categories that will not surface under Consumer-biased discovery.
Sub-scan sequence for every discovery pass, v0.5 onward: (1) sub-sector scan per ICP Section 4.2, (2) use-case scan per the ten Vision patterns, (3) incumbent-customer scan across the six named processors, (4) category-driven scan across the four Card-Program-Categories headings. Consolidate the four sub-scan outputs, deduplicate, then apply Section 8 scoring. Every candidate that lands in the Qualification Matrix from v0.6 onward carries a Card Program Category value drawn from the four canonical categories.
Structural axes as auxiliary characterisation: the six axes in Card-Program-Categories Section 2 (funding, cardholder, form factor, acceptance loop, currency, processing) are not scored parameters but are commercially relevant. Funding model in particular drives working capital and regulatory burden. Where a candidate's program is unusual on one of these axes (e.g., just-in-time funded prepaid for on-demand delivery, multi-currency wallet for FX-led travel), note it in the Trigger Notes column against the account record so future outreach lands with the right commercial framing.

### 4.1.5 Early-stage Watch List
Discovery weighted toward companies with visible card programs misses pre-launch fintechs — those between founding and their first public event, typically a 6 to 24 month window. These companies are picking their infrastructure now, have zero switching cost, and will need an issuer processor within 6 to 18 months. TXN's market-entry window in September 2026 and first-live-client capability in December 2026 align well with a Series A cohort targeting Q2 to Q4 2027 launch. Losing this cohort to competitors during our pre-launch period is the largest single missed-pipeline risk.
The Watch List captures pre-Prospect candidates who show at least one of five signals: (1) alumnus or current participant in a recognised accelerator (Y Combinator, Techstars, Seedcamp, Antler, Startupbootcamp FinTech, Rockstart, Founders Factory, ReaktorX, MIT Enterprise Forum CEE); (2) fresh EMI, PI, or Small Payment Institution licence granted or applied for in the last 6 months on EBA, KNF, CNB, ANAF, MNB, BaFin, ACPR, FCA, CSSF, or Bank of Lithuania registers; (3) featured in a Sifted, Finextra, TheRecursive, Business Insider CE, or Fintech Futures "startups to watch" piece in the last 6 months; (4) LinkedIn public job posting or founder announcement of a first "Head of Payments", "Head of Card Programs", "Head of Issuing", or "VP Payments" hire at a company with fewer than 30 employees; (5) pre-Series-A funding round PR that mentions card issuance, BIN sponsor, or issuer processor in the announcement text.
Watch List size ceiling: companies over 100 employees do not belong on the Watch List and instead go straight to the main Scoring Register as a full Prospect. Watch List is for the pre-scale cohort where the value is having the record captured before the company appears on Dealroom or Crunchbase, not for right-sized late-stage candidates.
Storage: the Watch List is held in 10_Watch_List of the Qualification Matrix. Watch_ID uses the WATCH- prefix (WATCH-001, WATCH-002, and so on). No auto-scoring is applied at Watch List stage. Fields captured: Company Name, Founded Date, HQ Country, Sub-sector best-guess, Card Program Category best-guess, Signal Source, Signal Detail, Signal Date, Product Summary, Promote Trigger, Source URL, Notes, Promote-to-Prospect Readiness (Not Ready / Ready / Promoted), Owner Review Status (Pending / Reviewed / Skipped), Last Reviewed Date, Promoted Account_ID.
Cadence: Claude runs a monthly Watch List scan on the first working day of each month, using web search plus the specific source list above. Output is a delta of net-new candidates from the previous month's scan. Owner reviews within one working week. Weekly promote-check by the Owner: any Watch List entry that has hit its Promote Trigger (announced funding round close, published licence issuance, hired first Head of Payments, appeared on a second "watch" list, or otherwise crossed a defined threshold) moves to the main Scoring Register as a full Prospect with a fresh Account_ID (country prefix per Section 4.1.3), and the Promoted Account_ID field links back to the Watch List row.
Signal quality caveat: early-stage signals are lower precision than main-register signals. Some Watch List entries will never mature to a card programme; a subset will be acquired, will pivot, or will fail. That is expected and is not a reason to lower the bar on which candidates we log. The value is having the record captured so that when they DO mature, TXN responds within days rather than months. A Watch List entry that has sat with no promote-trigger for 24 months is archived to a dormant state.
